...
class SNPTable(object):
    ...
    def get_overlapping_snps_from_bedpe(self, pos, cigar, seqlen, seq, cigarstring):
        """Returns several lists: 
        [1] indices of SNPs that this read overlaps,
        [2] positions in read sequence that overlap SNPs, 
        [3] indices for indels that read overlaps, 
        ...
        for (op, op_len) in cigar:
            # op = cig[0] # CIGAR 'operation'
            # op_len  = cig[1] # length of operation
        ...
        if read_end != seqlen:
            raise ValueError("length of read segments in CIGAR %d "
                             "does not add up to query length (%d), CIGAR=%s, seq=%s, cigarstring=%s" %
                             (read_end, seqlen, cigar, seq, cigarstring))
            raise ValueError("length of read segments in CIGAR %d "
                             "does not add up to query length (%d)" %
                             (read_end, seqlen))
