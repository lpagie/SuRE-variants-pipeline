...
BCCOUNTFNAME=${OUTDIR}/${BASENAME}_BCcounts.txt
...
# cat ${INFO} | awk -F "\t" '$2>-1' | cut -f 5 | sort -S 50% | uniq -c | \ # this filter was meant to discard reads without adapter. But allowing such reads will contribute to statistics (length(BC)==0, more??)
...
tee >( gzip -c > ${TABLE}) | \
cut -f 1 | sort | uniq -c | sort -k2g > ${BCCOUNTFNAME} 

# create plots in R with distributions of simple statistics:
echo "Create distribution plots of some data statistics:"
R --no-save -q <<EOR 
  # distribution of BC counts
  fname <- "${BCCOUNTFNAME}";
  oname <- sub('_BCcounts.txt','_BCcount_distr.png',fname)
  main <- "${BASENAME}";
  dt <- scan(what=list(1,1), file=fname)
  main <- sprintf("%s: BC counts, cDNA (# correct BC = %d)", main, as.integer(sum(dt[[1]]*dt[[2]])))
  ...
  # distribution of BC lengths
  fname <- "${BCLENGTHFNAME}"
  oname <- sub('_BClengths.txt','_BClength_distr.png',fname)
  main <- "${BASENAME}";
